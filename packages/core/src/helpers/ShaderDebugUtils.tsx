...
function parseGlobals(prefix: string, text: string) {
...
function parseLocalVariables(text: string) {
  const result: any[] = [];
  const mainRegex: any = /void\s*main\s*\(.*?\)[\s\S]*\{/;
...
function parseDeclarations(body: string, startIndex: number, endIndex: number) {
...
export function lineColToIndex(text: string, line: number, column: number) {
...
export function getScopeDepth(_: any, index: number) {
  const braceRegex = /[{}]/g;
  let braceCount = 0;
  let lastResult: any;
...
export function getMainExtents(text: string) {
  ...
  const mainResult: any = mainRegex.exec(text);
